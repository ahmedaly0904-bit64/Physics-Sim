...
- **`mProject_0/` — Projectile motion**
  ...
  - The theoretical trajectory of a projectile launched with initial speed `v0` and angle `θ` (from horizontal) is:
    - `x(t) = v0 cos(θ) · t`
...
- **`mProject_1/` — Random walk**
  ...
  - Demonstrates that the typical displacement after `N` steps scales like:
    - `<r²>^(1/2) ∝ √N`
    which is a fundamental result in diffusion and stochastic processes.

- **`Numerical Analysis/` — Numerical methods and dynamical systems**
  - **`Numerical_Integration.ipynb`**: Approximates integrals of the form
    - `I = ∫_a^b f(x) dx`
    using the trapezoidal and Simpson rules, and compares numerical errors as the step size `h` is refined.
  - **`Root_Finding.ipynb`**: Solves nonlinear equations `f(x) = 0` using bisection and Newton–Raphson iterations, for example
