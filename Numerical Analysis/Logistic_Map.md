...
- `x_n ∈ [0, 1]` represents a normalized population at step `n`,
...
- **1 < r < 3**: Convergence to a non‑zero fixed point:
  - `x* = 1 − 1/r`.
- **3 < r ≲ 3.57**: Period‑doubling cascade—stable orbits of period `2, 4, 8, …`.
- **r ≳ 3.57**: Onset of chaos, with windows of periodicity embedded in chaotic regions.
...
2. For each `r`:
   - Iterates the map many times starting from an initial `x0`.
...
- Change the initial condition `x0` and verify that long‑term behavior in chaotic regions becomes independent of `x0`.
